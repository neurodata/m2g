...
duecredit==0.7.0 , fury==0.3.0 , graspy==0.0.3 , ipython==7.7.0 , matplotlib==3.1.1 , networkx==2.3 , nibabel==2.5.0 , 
...
suffixes:
reor = 
RAS = File has been reoriented into RAS+ orientation
nores = The file originally had the desired voxel size, thus resulting in no reslicing

output
     anat
          preproc
               Files created during the preprocessing of the anatomical data
                   t1w_aligned_mni.nii.gz = t1w anatomical image in mni space
                   t1w_brain.nii.gz = t1w anatomical image with everything aside from the brain removed
                   ...
                   t1w_seg_pve_0_nores.nii.gz/t1w_seg_pve_0_res.nii.gz = 
                   t1w_seg_pve_1.nii.gz = grey matter
                   t1w_seg_pve_1_nores.nii.gz/t1w_seg_pve_1_res.nii.gz = 
                   t1w_seg_pve_2.nii.gz = white matter
                   t1w_seg_pve_2_nores.nii.gz/t1w_seg_pve_2_res.nii.gz = 
                   t1w_seg_pveseg.nii.gz = 
                   t1w_wm_thr.nii.gz = 
                   
          registered
               Files created during the registration process
                   t1w_corpuscallosum.nii.gz
                   t1w_corpuscallosum_dwi.nii.gz
                   t1w_csf_mask_dwi.nii.gz
                   t1w_gm_in_dwi.nii.gz
                   t1w_in_dwi.nii.bz
                   t1w_wm_gm_int_in_dwi.nii.gz
                   t1w_wm_gm_int_in_dwi_bin.nii.gz
                   t1w_wm_in_dwi.nii.gz
     ...
     dwi
          ...
          preproc
               Files created during the preprocessing of the dwi data
                    ...
                    nodif_B0.nii.gz
                    nodif_B0_bet.nii.gz
                    nodif_B0_bet_mask.nii.gz
                    bvec.bvec
                    bvec_scaled.bvec
                    bvecs_reor.bvecs
                    bval.bval
                    eddy_corrected_data.nii.gz
                    eddy_corrected_data.ecclog
                    eddy_corrected_data_reor_RAS.nii.gz
                    eddy_corrected_data_reor_RAS_res.nii.gz
          roi-connectomes
               Location of connectome(s) created by the pipeline, with a directory given to each atlas you use for your analysis
          tensor
               Used in mni analysis?
     qa
          adjacency
          fibers
          graphs
     ...
     tmp
          reg_a
               desikan_space-MNI152NLin6_res-2x2x2_reor_RAS_nores_aligned_atlas_skull.nii.gz
               t1w_csf_mask_dwi_bin.nii.gz
               t1w_vent_mask_dwi.nii.gz
               vent_mask_mni.nii.gz
               desikan_space-MNI152NLin6_res-2x2x2_reor_RAS_nores_aligned_atlas_t1w_mni.nii.gz
               t1w_gm_in_dwi_bin.nii.gz
               t1w_wm_edge.nii.gz
               vent_mask_t1w.nii.gz
               mni2t1w_warp.nii.gz
               t1w_vent_csf_in_dwi.nii.gz
               t1w_wm_in_dwi_bin.nii.gz
               warp_t12mni.nii.gz
          ...
          reg_m
               ...
               t1w2dwi_bbr_xfm.mat
               t1w2dwi_xfm.mat
               t1wtissue2dwi_xfm.mat
               xfm_mni2t1w_init.mat
               xfm_t1w2mni_init.mat
```
...
CHECK THAT THIS COMMAND WORKS:

    docker run -ti -v /path/to/local/data:/data bids/ndmg /data/ /data/outputs

## Working with S3 Datasets
**ndmg** has the ability to work on datasets stored on [Amazon's Simple Storage Service](https://aws.amazon.com/s3/), assuming they are in BIDS format. To 
