...
Pillow==6.1.0 , plotly==1.12.9, pybids==0.6.4 , python-dateutil==2.8.0 , PyVTK==0.5.18 , requests==2.22.0 , s3transfer==0.2.1 , setuptools>=40.0 scikit-image==0.13.0 , scikit-learn==0.21.3 , scipy==1.3.0 ,
sklearn==8.0 , vtk==8.1.2
```
...
In order to create a docker container from the docker image and enter it, use the following command to both create and enter the container:
...
nores = File originally had the desired voxel size specified by the user, thus resulting in no reslicing
...
```
Other files may end up in the output folders, depending on what settings or atlases you choose to use. Files may also be removed if the ```--clean``` setting is used.
...
**ndmg** has the ability to work on datasets stored on [Amazon's Simple Storage Service](https://aws.amazon.com/s3/), assuming they are in BIDS format.
