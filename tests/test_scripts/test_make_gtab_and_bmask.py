...
def test_make_gtab_and_bmask(tmp_path): 
	...
	fbvec = '../test_data/inputs/bvec.bvec'
	...
	outdir = '../test_data/temp_outputs'

	#define correct output data path 
	# stored as a numpy array 
	gtab_grad_path = '../test_data/outputs/gtab/gtab_grad'
	gtab_bvals_path = '../test_data/outputs/gtab/gtab_bvals'
	gtab_bvecs_path = '../test_data/outputs/gtab/gtab_bvecs'
	gtab_b0s_mask_path = '../test_data/outputs/gtab/gtab_b0s_mask'
	gtab_b0_threshold_path = '../test_data/outputs/gtab/gtab_b0_threshold'

	nodif_B0_path = '../test_data/outputs/nodif_B0.nii.gz'
	nodif_B0_mask_path = '../test_data/outputs/nodif_B0_bet_mask.nii.gz'

	#load data

	#each property of gtab object 
	#unable to directly save unique dipy Gradient Table Class
	gtab_grad_control = np.loadtxt(gtab_grad_path)
	gtab_bvals_control = np.loadtxt(gtab_bvals_path)
	gtab_bvecs_control = np.loadtxt(gtab_bvecs_path)
	gtab_b0s_mask_control = np.loadtxt(gtab_b0s_mask_path)
	gtab_b0_threshold_control = np.loadtxt(gtab_b0_threshold_path)

	#load control as np.arrays
	nodif_B0_control = nib.load(nodif_B0_path).get_fdata()
	nodif_B0_mask_control = nib.load(nodif_B0_mask_path).get_fdata()
	...
	nodif_B0_test = nib.load(nodif_B0).get_fdata()
	nodif_B0_mask_test = nib.load(nodif_B0_mask).get_fdata()

	print(gtab_b0_threshold_control)
	print(gtab.b0_threshold )
	...
	assert gtab.b0_threshold ==  gtab_b0_threshold_control

	#test b0
	assert np.allclose (nodif_B0_test,nodif_B0_control )
	assert np.allclose (nodif_B0_mask_test, nodif_B0_mask_control)
