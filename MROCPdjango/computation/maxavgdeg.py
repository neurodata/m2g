...
def getMaxAveDegree(G_fn, lcc_fn = None, roiRootName = None, toDir = None, saveTop50 = True):
  '''
  ...
  if toDir:
    eigvl_fn = os.path.join(toDir, getBaseName(G_fn) + '_eigvl.npy')
    MAD_fn = os.path.join(toDir, getBaseName(G_fn) + '_MAD.npy')
  ...
  numEigs = 50 if (G.shape[0] > 51) else G.shape[0]-2 # Number of eigenvalues to compute
...
def main():
    ...
    parser.add_argument('-s', '--saveTop50', help='Save the top 50 results to a file' )
    
    result = parser.parse_args()
    if (results.saveTop5):
      getMaxAveDegree(result.G_fn, result.lcc_fn, result.roiRootName, result.toDir)
    else:
      getMaxAveDegree(result.G_fn, result.lcc_fn, result.roiRootName, result.toDir, False)
