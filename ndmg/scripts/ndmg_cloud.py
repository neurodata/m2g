...
def main():
    ...
    parser.add_argument(
        "state",
        choices=["participant", "group", "status", "kill"],
        default="paricipant",
    ...
    parser.add_argument(
        "--stc",
        action="store",
        choices=["None", "interleaved", "up", "down"],
        default="None",
        help="The slice timing " "direction to correct. Not necessary.",
    )
    parser.add_argument(
        "--modality",
        action="store",
    ...
    if state == "status":
        ...
        batch_submit(
            bucket, path, jobdir, creds, state, debug, dset, log, stc, mode, bg
        )
