...
#
# Usage:
#    ./runndmg.py /path/to/rest/files/by/line /path/to/anat/files/by/line /path/to/output/dir
    ...
    with open(results.restfile) as restfile:
        with open(results.anatfile) as anatfile:
            ...
            for (rest, anat) in zip(restpaths, anatpaths):
                try:
                    p = Process(target=fmri_pipeline, args=(rest, anat, atlas, atlas_brain, mask, labels,
                                  results.outdir), kwargs={'clean':False, 'fmt':'graphml'})
                    p.start()
                    p.join()
