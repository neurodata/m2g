...
def get_atlas(atlas_dir, modality, vox_size):
    ...
    if modality == "dwi":
        ...
        atlas_mask = op.join(
            atlas_dir, "mask/MNI152NLin6_res-" + dims + "_T1w_descr-brainmask.nii.gz"
        )
    ...
    for f in fils:
        if not ope(f):
            print(f)
    if any(not os.path.exists(f) for f in fils):
        print("Cannot find atlas information; downloading...")
        os.system("mkdir -p " + atlas_dir)
        cmd = "wget https://github.com/neurodata/neuroparc/archive/master.zip"
        os.system(cmd)
        cmd = "unzip {}/master.zip".format(os.getcwd())
        os.system(cmd)
        os.rename(
            "{}/neuroparc-master/atlases".format(os.getcwd()),
            "{}/ndmg_atlases".format(os.getcwd()),
        )
        shutil.rmtree("{}/neuroparc-master".format(os.getcwd()))
